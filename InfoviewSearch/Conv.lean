...
/-!
# Mostly a copy-paste of the corresponding Mathlib file

TODO: modify the version in Mathlib so that it exports the thing that I need.
-/
...
-/
def pathToStx {m} [Monad m] [MonadEnv m] [MonadQuotation m] (convStx : TSyntax `conv)
...
/-- Generate a `conv => ...` wrapper around the given conv tactic `convStx` to create
a tactic that executes `convStx` at the specified subexpression. -/
def generateConvSyntax (convStx : TSyntax `conv)
    (expr : Expr) (subexprPos : SubExpr.Pos) (fvarUserName? : Option Name) :
    MetaM (TSyntax `tactic) := do
  let path ← Path.ofSubExprPosArray expr subexprPos.toArray
  pathToStx convStx path fvarUserName?

end InfoviewSearch.Conv
